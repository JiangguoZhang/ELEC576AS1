...
class DatasetLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, name_list):
        cell_data, cell_label = datasets.load_wine(return_X_y=True)
        cell_data = (cell_data - np.mean(cell_data, axis=0) + 1) / np.std(cell_data, axis=0)
        self.cell_label = torch.LongTensor(cell_label)
        self.cell_data = torch.FloatTensor(cell_data)
...
def construct_linear_block(nn_input_dim, nn_output_dim, actFun_type):
    ...
    if actFun_type == "tanh":
        ...
        sequential.append(("softmax", nn.Softmax(dim=1)))
...
def main(args):
    ...
    model = DeepNeuralNetwork(nn_input_dim=args.input_dim, nn_num_layers=args.num_layers, nn_hidden_dim=args.hidden_dim,
                              nn_output_dim=args.output_dim, actFun_type='sigmoid')
    model.cuda()
    loss = nn.NLLLoss()
    ...
    optimizer = optim.SGD(model.parameters(), momentum=0, lr=0.01)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_dim", type=int, default=13, help='input dimension')
    parser.add_argument("--num_layers", type=int, default=5, help='the number of layers')
    parser.add_argument("--hidden_dim", type=int, default=3, help='the number of hidden units')
    parser.add_argument("--output_dim", type=int, default=3, help='the number of output units')
    parser.add_argument("--output_dir", type=str, default="wine", help='the output directory')
